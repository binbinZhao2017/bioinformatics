...
def single_sv_compare(sv_file, config_file, output_dir):
    """
    Comparison of SV in single condition
    sv_file：sv vcf
    config_file：config file for filter, format json
    ...
    with open(sv_file, "r") as f:
        ...
        for line in lines:
            ...
            if line.startswith("#CHROM"):  # index of sample
                for i in range(9, len(item)):
                    sample_index[item[i]] = i
            if item[6] != "PASS":
                continue
            if region_info != "all":  # region filter
            ...
            for i in range(len(ty)):
                type_index[ty[i]] = i
            m = re.match(r".+;SVTYPE=(.+?);.+END=(\d+);.+", item[7])  # length calculation
            sv_type = m.group(1)
            end = m.group(2)
            ...
            for cs in sample_info.keys():
                ...
                compare_type = sample_info[cs]["compare_type"]
...
def multiple_sv_compare(sv_file, config_file, output_dir):
    ...
    """
    Comparison of SV in multiple condition
    sv_file：sv vcf
    config_file：config files for filter, format json
    output_dir: 输出结果文件夹
    """
    sample_info = json.loads(open(config_file).read())
    if "Region" in sample_info.keys():
    ...
    with open(sv_file, "r") as f:
        ...
        for line in lines:
            ...
            if break_flag:
                continue
            if "PASS" not in ft_list:
                continue
            # print item[:3]
            m = re.match(r".+;SVTYPE=(.+?);.+END=(\d+);.+", item[7])
            sv_type = m.group(1)
            end = m.group(2)
            sample_genotype = sv_type if s_ft == "PASS" else "-"
            c_sample_genotype = sv_type if cs_ft == "PASS" else "-"
